...
namespace ISO8583Engine {
            ...
            get {
                if (this.FieldHeaderLength > 0) {
                    ...
                    return (string.IsNullOrEmpty(this.FieldValue)) ? this.FieldValue :
                        ((this.PaddingPlacement.Equals(Padding.Left)) ?
    ...
    public class ISO8583Message {
        ...
        internal static Dictionary<isoMessageElement, int> isoFieldLengths = new Dictionary<isoMessageElement, int>();
        //TODO: Remove the dependency on the FieldHeaderLength Dictionary, Do we really require it?
        internal static Dictionary<isoMessageElement, int> FieldHeaderLength = new Dictionary<isoMessageElement, int>();
        ...
        internal int GetFieldHeaderLength(isoMessageElement Index) {
            try {
                return ISO8583Message.FieldHeaderLength[Index];
            } catch (Exception) {
                return 0;
            }
        }
        ...
        private bool ReceiveAndParseResponse(Stream DataStream) {
            ...
            try {
                ...
                string BitmapString = GetBitmapString(BitmapBytes, BitmapLength);
                LogMessage("Bitmap string = " + BitmapString);
        ...
        public string DumpISO8583State() {
            ...
            for (int i = 0; i < 128; i++) {
                isoMessageElement tmp = (isoMessageElement)i;
                if (this.IsoFields.ContainsKey(tmp)) {
                    Message.AppendLine(string.Format("{0}='{1}'", tmp, IsoFields[tmp].FieldValue));
                }
            }
